        ...
        private static async void Bot_OnMessageReceived(object sender, Telegram.Bot.Args.MessageEventArgs e)
        {
            ...
            switch (message.Text)
            {
                case "/start":
                    Bot.StartReceiving();
                    string text =
@"Привет, моя любимая Лерочка! Я бот твоего любимого мужика. Через меня ты можешь написать своему любимому в ВК или отправить какую-нибудь фоточку, которая его порадует :)
...
/stop - ост ановить бота";
                ...
                case "/stop":
                    await Bot.SendTextMessageAsync(message.From.Id, "До встречи, красавица!");
                    Bot.StopReceiving();
